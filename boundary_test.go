...
func TestBoundary_UnixEpoch(t *testing.T) {
	...
	parser := mustCronParser(t, "* * * * *", WithLocation(time.UTC))
	next := parser.Next(epoch)
...
func TestBoundary_Year1970(t *testing.T) {
	...
	parser := mustCronParser(t, "30 12 * * *", WithLocation(time.UTC))
	next := parser.Next(earlyTime)
...
func TestBoundary_LeapYear_Feb29(t *testing.T) {
	...
	parser := mustCronParser(t, "0 0 29 2 *", WithLocation(time.UTC))
...
func TestBoundary_MonthEnd_AllMonths(t *testing.T) {
	...
	parser := mustCronParser(t, "0 0 L * *", WithLocation(time.UTC))
...
func TestBoundary_Day31_ShortMonths(t *testing.T) {
	parser := mustCronParser(t, "0 0 31 * *", WithLocation(time.UTC))
...
func TestBoundary_YearEnd(t *testing.T) {
	...
	parser := mustCronParser(t, "0 0 * * *", WithLocation(time.UTC))
	next := parser.Next(yearEnd)
...
func TestBoundary_Seconds_Max(t *testing.T) {
	...
	parser := mustCronParser(t, "* * * * * *", WithSeconds(), WithLocation(time.UTC))
	next := parser.Next(time59)
...
func TestBoundary_Seconds_StepValues(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.expr, func(t *testing.T) {
			...
			parser := mustCronParser(t, tc.expr, WithSeconds(), WithLocation(time.UTC))
			next := parser.Next(start)
...
func TestBoundary_7Fields_Complete(t *testing.T) {
	...
	parser := mustCronParser(t, "45 30 10 15 3 * 2025-2030", WithSeconds(), WithYears(), WithLocation(time.UTC))

	// Test from before the range
	start2024 := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	next := parser.Next(start2024)
	...
	for year := 2025; year <= 2030; year++ {
		...
		yearNext := parser.Next(yearStart)
	...
	afterRange := parser.Next(start2031)
...
func TestBoundary_7Fields_YearRange(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.yearExpr, func(t *testing.T) {
			...
			parser := mustCronParser(t, expr, WithSeconds(), WithYears(), WithLocation(time.UTC))
			next := parser.Next(tc.start)
...
func TestBoundary_ImpossibleDate(t *testing.T) {
	...
	parser := mustCronParser(t, "0 0 30 2 *", WithLocation(time.UTC))
...
func TestBoundary_WeekdayAndMonthday(t *testing.T) {
	...
	parser := mustCronParser(t, "0 0 15 * 5", WithLocation(time.UTC))
...
func TestBoundary_MinuteRollover(t *testing.T) {
	parser := mustCronParser(t, "0 * * * *", WithLocation(time.UTC))
