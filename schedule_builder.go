...
func (sb *ScheduleBuilder) Do(job any, taskID ...string) error {
	...
	var wrappedJob Job
	var id string
	autoGeneratedID := len(taskID) == 0 || taskID[0] == ""
	...
	task, err := buildTask(cronExpr, id, wrappedJob, sb.options...)
	if err != nil {
		return err
	}

	return sb.scheduler.commitTask(task, autoGeneratedID)
}
...
func (sb *ScheduleBuilder) buildCronExpression() (string, error) {
	if sb.interval <= 0 {
		return "", fmt.Errorf("interval must be positive, got %d", sb.interval)
	}
